...
class AtlasI2C:
    ...
    def continuouspolling(self,delaytime):
        ...
        try:
            while True:
                print(self.query("R"))
                time.sleep(delaytime - self.long_timeout)
        except KeyboardInterrupt:
            print("\nContinuous polling stopped")

class pHandDOSensors(object):
    ...
    def pH(self):
        result = self.pHSensor.query("R")
        if result[:18] == "Command succeeded ":
            numResult = float(result[18:].rstrip('\0'))
            return numResult
        else: print("Error: Unable to get pH value")
